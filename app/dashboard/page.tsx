  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const ownedBusinesses = biz || []
        const businessIds = ownedBusinesses.map((b) => b.id)

        let reviewStatsMap: Record<string, { count: number; avg: number }> = {}
        ...
        if (businessIds.length > 0) {
          ...
          if (!reviewsError && reviewsData) {
            ...
            reviewsData.forEach((r: any) => {
              ...
              if (!tempMap[r.business_id]) {
                tempMap[r.business_id] = { sum: 0, count: 0 }
              }

              tempMap[r.business_id].sum += Number(r.rating || 0)
              tempMap[r.business_id].count += 1
            })

            Object.keys(tempMap).forEach((businessId) => {
              const row = tempMap[businessId]
              reviewStatsMap[businessId] = {
                count: row.count,
        ...
        const mergedBusinesses = ownedBusinesses.map((b) => ({
          ...b,
          review_count: reviewStatsMap[b.id]?.count || 0,
          avg_rating: reviewStatsMap[b.id]?.avg || 0,
        }))
      ...
      <div className="px-4 pt-4 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 text-center">
            ...
            <div className="mt-3">
              <Link
                ...
                className="block w-full bg-slate-100 text-slate-700 py-2.5 rounded-lg text-[13px] font-bold"
              >
                내 업소 찾기로 이동
              </Link>
            </div>
          ...
          businesses.map((b) => (
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <Link
                  href={`/pricing?business_id=${b.id}`}
                  className="block w-full text-center bg-amber-400 text-[#1a1a2e] py-2 rounded-lg text-[13px] font-bold"
                >
                  VIP 신청
                </Link>
              </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <div className="text-[12px] text-slate-500 space-y-1">
            <div>• 내 업소의 최근 리뷰를 확인할 수 있습니다.</div>
            <div>• VIP 신청으로 상단 노출을 강화할 수 있습니다.</div>
            <div>• 정보 수정 요청은 관리자 승인 후 반영됩니다.</div>
          </div>
        </div>
      </div>
    </div>
