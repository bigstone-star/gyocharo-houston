...
export default function AdminBusinessEditPage({ params }: { params: { id: string } }) {
  ...
  const save = async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const nextMetroArea =
        inferMetroArea(nextCity, biz.address || '') || biz.metro_area || null
      ...
      alert('업소 정보가 저장됐습니다. 확인을 누르면 목록으로 이동합니다.')
  ...
  return (
    <div className="min-h-screen bg-slate-100 max-w-2xl mx-auto pb-10">
      ...
      <form onSubmit={save} className="px-4 py-5 space-y-4">
        <div className="sticky top-[72px] z-20">
          <div className="bg-white/95 backdrop-blur border border-slate-200 rounded-xl px-4 py-3 flex items-center justify-between shadow-sm">
            <div>
              <div className="text-[15px] font-bold text-slate-800">
                {biz?.name_kr || biz?.name_en || '업소 수정'}
              </div>
              <div className="text-[12px] text-slate-500">
                상단 저장 버튼으로 바로 저장할 수 있습니다.
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={saving}
              className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-[14px] font-bold disabled:opacity-60"
            >
              {saving ? '저장 중...' : '저장하기'}
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                value={
                  inferMetroArea(
                    normalizeCityFromAddress(biz?.address || '') || biz?.city || '',
                    biz?.address || ''
                  ) ||
                  biz?.metro_area ||
                  ''
